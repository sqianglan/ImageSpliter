...
// Note: IJM dialogs do not support true tabs, so this package uses a top-level workflow selector.
...
function run_folder_review() {
    ...
    reviewEntriesText = collect_review_entries(inputDir);
    if (reviewEntriesText == "") {
        exit("No supported image files were found in the selected folder.");
    }
    reviewEntries = split(reviewEntriesText, "\n");

    for (reviewIndex = 0; reviewIndex < reviewEntries.length; reviewIndex++) {
        if (reviewEntries[reviewIndex] == "") continue;
        reviewParts = split(reviewEntries[reviewIndex], "\t");
        reviewPath = reviewParts[0];
        reviewSeries = parseInt(reviewParts[1]);

        run("Bio-Formats Importer", "open=[" + reviewPath + "] color_mode=Default rois_import=[ROI manager] view=Hyperstack stack_order=XYCZT series_" + reviewSeries);
        getPixelSize(U, px, py);
        run("Set Scale...", "distance=" + 1/px + " known=1 unit=" + U);

        reviewResult = review_current_image(reviewPath, reviewSeries);
        close("*");
...
function apply_review_position_and_colors(previewChannel, previewSlice) {
    getDimensions(width, height, channels, slices, frames);
    if (previewChannel < 1) previewChannel = 1;
    if (previewSlice < 1) previewSlice = 1;
    if (channels >= previewChannel) Stack.setChannel(previewChannel);
    if (slices >= previewSlice) Stack.setSlice(previewSlice);
    apply_review_preview_colors();
}
...
function export_current_slice_outputs(outputBase, sliceNumber) {
    ...
    mergedCyanCh = cyanCh;
    mergedMegaCh = megaCh;
    mergedYellowCh = yellowCh;
    currentImageName = sanitize_review_output_name(File.getName(rawPath));
    imageDir = File.getParent(rawPath);

    if (zSplitChannels) {
        splitProcessingTitle = "Z_Split_Processing";
        if (isOpen(splitProcessingTitle)) {
            selectWindow(splitProcessingTitle);
            close();
        }
        selectImage(rawID);
        run("Duplicate...", "title=" + splitProcessingTitle);
        selectWindow(splitProcessingTitle);
    ...
    run("RGB Color");
    saveAs("Tiff", outputBase + "_z" + sliceNumber + "_merged_rgb.tif");
    close();
...
function split_channel_index(title) {
    dashIndex = indexOf(title, "-");
    if (dashIndex > 1 && substring(title, 0, 1) == "C") {
        return parseInt(substring(title, 1, dashIndex));
    }
    return 1;
}
...
function build_review_output_base(reviewPath, reviewSeries) {
    ...
    reviewFileName = sanitize_review_output_name(File.getName(reviewPath));
    return reviewOutputFolder + File.separator + reviewFileName + "_s" + reviewSeries;
}

function collect_review_entries(currentDirectory) {
    result = "";
    ...
    for (j = 0; j < localList.length; j++) {
        ...
        if (is_supported_review_file(localList[j])) {
            ...
            result += collect_review_entries(localPath);
        }
    }
    return result;
}
...
function run_batch_export(workflow) {
    ...
    if (workflow == "Multichannel Image" && channelSplit) {
        Dialog.create("Merged File Settings");
        Dialog.addNumber("Gray channel", grayCh);
        Dialog.addNumber("Cyan channel", cyanCh);
        Dialog.addNumber("Magenta channel", megaCh);
        Dialog.addNumber("Yellow channel", yellowCh);
...
function run_z_slice_export() {
    ...
    Dialog.addMessage("Folder Review: open files one by one, preview normally, extract current z, then load next.");
    Dialog.show();
    ...
    if (t < 0) {
        ...
        currentImageName = substring(name, 0, t);
    }

    export_current_slice_outputs(outputDir + currentImageName, z);
}
    ...
    for (file = 0; file < fileList.length; file++) {
        if (endsWith(fileList[file], fileExtension)) {
            ...
            for (series = 1; series <= seriesCount; series++) {
                ...
                if (imageDir == dir) {
                    outputFolder = parentDir + File.separator;
                } else {
                    outputFolder = dir;
                }
                if (outputFolder.contains(dirName)) {
                    ...
                    outputFolder = outputFolder + dirName + "_Image_Splitted_" + timeStamp();
                }
...
function color_channels(grayCh, yellowCh, cyanCh, megaCh) {
    if (grayCh != 0) { Stack.setChannel(grayCh); run("Grays"); }
    if (yellowCh != 0) { Stack.setChannel(yellowCh); run("Yellow"); }
    if (cyanCh != 0) { Stack.setChannel(cyanCh); run("Cyan"); }
    if (megaCh != 0) { Stack.setChannel(megaCh); run("Magenta"); }
}
...
function split_channel_save(outputBaseName) {
    ...
    for (channel = 0; channel < list.length; channel++) {
        ...
        fileNameChannel = outputBaseName + "_Ch" + (channel+1) + ".tif";
...
function merge_channel(outputBaseName) {
    list = getList("image.titles");
    channelValues = newArray(mergedGrayCh, mergedCyanCh, mergedMegaCh, mergedYellowCh);
    count = 0;
    for (i = 0; i < channelValues.length; i++) if (channelValues[i] != 0) count++;
    ...
    for (i = 0; i < channelValues.length; i++) {
        if (channelValues[i] != 0) {
            cmd += "c" + (i+4) + "=[" + list[(channelValues[i]-1)] + "] ";
...
function sanitize_output_name(name, fileExtension) {
...
function sanitize_review_output_name(name) {
    currentImageName = strip_known_review_extension(name);
    currentImageName = replace(currentImageName, "-", "_");
    currentImageName = replace(currentImageName, "\\", "_");
    currentImageName = replace(currentImageName, ":", "_");
    currentImageName = replace(currentImageName, "*", "_");
...
function strip_known_review_extension(name) {
    lowerName = toLowerCase(name);
    if (endsWith(lowerName, ".ome.tiff")) return substring(name, 0, lengthOf(name) - 9);
    if (endsWith(lowerName, ".ome.tif")) return substring(name, 0, lengthOf(name) - 8);
    t = lastIndexOf(name, ".");
    if (t < 0) return name;
    return substring(name, 0, t);
}
